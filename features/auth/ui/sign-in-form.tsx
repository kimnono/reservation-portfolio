...
import { demoAccounts } from "@/features/auth/api/mock";
import { login } from "@/features/auth/api/login";
import { getRoleLabel } from "@/features/auth/model/roles";
import {
  signInSchema,
  type SignInValues,
} from "@/features/auth/model/sign-in-schema";
    ...
    <section className="mx-auto grid min-h-[calc(100vh-5rem)] w-full max-w-7xl gap-6 px-6 py-10 lg:grid-cols-[1.25fr_0.95fr]">
      ...
      <Card className="self-center">
        ...
        <form className="mt-8 space-y-5" onSubmit={handleSubmit(onSubmit)}>
          <div className="grid gap-3 md:grid-cols-3">
            {demoAccounts.map((account) => (
              <button
                ...
                className={button.secondaryTile}
              >
          ...
          <button className={button.primaryWide} type="submit" disabled={isSubmitting}>
